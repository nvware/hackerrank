...
class Result {
    ...
    public static String twoArrays(int k, List<Integer> A, List<Integer> B) {
        ...
        List<Integer> sortedOerderA = A.stream()
                .sorted()
                .collect(Collectors.toList());

        List<Integer> sortedReverseOrderB = B.stream()
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toList());

        int n = sortedOerderA.size();
        for (int i = 0; i < n; i++) {
            if (sortedOerderA.get(i) + sortedReverseOrderB.get(i) < k) {
                return "NO";
            }
        }
